...
Antena* InsereOrdenado(Antena* lista, char frequencia, int x, int y) {
    ...
    Antena* nova = CriaAntena(frequencia, x, y);
    if (!nova) return lista;

    if (!lista || (lista->y > y || (lista->y == y && lista->x > x))) {
        nova->next = lista;
        return nova;
    }

    Antena* a = lista; // inserir no meio
...
Antena* RemoveAntena(Antena* lista, int x, int y) {
    ...
    if (lista->x == x && lista->y == y) { //se estiver no inicio
    ...
    if (atual->next) { //se estiver no meio
...
Antena* ListaAntenas(Antena* lista, int* num_antenas) {
    *num_antenas = 0;
    ...
    Antena* resultado = (Antena*)malloc(sizeof(Antena) * (*num_antenas));
        ...
        for (Antena* b = a->next; b != NULL; b = b->next) {
            if (a->frequencia == b->frequencia) {
                if (a->x == b->x && abs(a->y - b->y) == 2) { //vertical
                    int temp = a->y;
                    a->y = (a->y < b->y) ? a->y - 2 : a->y + 2;
                    b->y = (temp < b->y) ? b->y + 2 : b->y - 2;
                    printf("Vertical: (%d, %d) e (%d, %d)\n", a->x, a->y, b->x, b->y);
                }
                else if (a->y == b->y && abs(a->x - b->x) == 2) { //horizontal
                    int temp = a->x;
                    a->x = (a->x < b->x) ? a->x - 2 : a->x + 2;
                    b->x = (temp < b->x) ? b->x + 2 : b->x - 2;
...
Antena* LerAntenas(const char* nome_ficheiro, Antena* lista) {
    ...
    for (int x = 0; fgets(linha, 250, ficheiro) != NULL; y++) {
...
bool GravaAntenasBinario(const char* nome_ficheiro, Antena* lista) {
    ...
    if (!ficheiro) {
        printf("Erro ao abrir o ficheiro %s para escrita binária\n", nome_ficheiro);
        return false;
    }
